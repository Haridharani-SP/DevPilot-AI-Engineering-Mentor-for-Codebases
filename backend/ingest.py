...
def load_github_repo(repo_url: str) -> List[Document]:
    """Clone and load a GitHub repository. Tries main, master, dev, develop."""
    import shutil
    ...
    for branch in ("main", "master", "dev", "develop"):
        try:
            ...
            print(f"  ✅ Loaded {len(docs)} files from branch '{branch}'")
            return docs
        except Exception as e:
            err = str(e)
            if "pathspec" in err or "exit code(1)" in err:
                if os.path.exists(clone_path):
                    shutil.rmtree(clone_path, ignore_errors=True)
                continue
    ...
    print(f"  ❌ No valid branch found for {repo_url}")
    return []
